...
#ifdef TARGET_PC
  /* PC: initialize viewport immediately so submenu restore has valid data
     even before showPerspectiveView runs. On GC, memory was likely zeroed. */
  set_viewport(&view->viewport, &view->screen);
#endif
