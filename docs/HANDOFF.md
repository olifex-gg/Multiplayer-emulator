...
**Eight residents (2026-09-16, option A of the "more than four" brainstorm; option C,
seats for any number of residents, is still to come).** `PLAYER_NUM` went from 4 to 8.
Stage 1, done and tested: the save, the server, the tools.

- *What grew.* `Save_t` is now 0x37620 (was 0x242A0), `Save` 0x38000, the others block
  0x32000 (the diary has eight residents' pages), the town file 0xA2040 (was 0x72040).
  `private_data`, `homes`, `mother_mail`, `keep_house_size`, the villagers' per-player
  `hp_mail`, the all-grow shine/stone positions (`TOTAL_PLAYER_NUM` = 9) and the kabu
  peddler's spoken ids all have eight (nine) entries. `mPr_FOREIGNER` is 8.
- *What could not just grow* (each packed players into bits sized for four):
  `house_arrangement` is a byte per player holding the house index (`m_house.c` rewritten;
  `mHS_HOUSE_NUM` 8, `mHS_HOUSE_MASK` 7, `mHS_HOUSES_PER_ACRE` 4); the saved-event flags
  (`mEv_event_save_c.flags`, `common_data.event_flags`) are `u64` with eight `_PLRn`
  entries per group; `LightHouse_c` has `players_quest_started` and `players_contributed`
  as separate bytes; K.K.'s `bitfield` is `u32` (HELLO bits 0-7, AIRCHECK 8-15, specials
  24-26); museum donors are 1-8 with `mMmd_DONATOR_DELETED_PLAYER` = 9; signboard ids are
  `START + player * 8 + design` with the blank at +64; the post office, museum and
  Valentine masks are `(1 << PLAYER_NUM) - 1`; the ~40 `& 3` masks on house and player
  indices became `mHS_HOUSE_MASK` / `PLAYER_NUM - 1`; the map's resident label names the
  four owners of each house acre (`mHS_house_acre_block`); a ninth face repeats one.
  `ANIMAL_MEMORY_NUM` stays 7: a villager remembers seven people, so with eight residents
  the one they talk to least drops out of memory (the original limit; a visible quirk).
- *Converting old towns.* `pc/src/pc_save_convert.c` turns a 0x72040 file into the new
  layout on load, field by field from `pc_save_legacy_layout.h` (generated by the scratch
  script `layout_dump2.py` + `gen_legacy_layout.py` from the four-player headers with the
  real compiler -- do not hand-edit it; every plain field is copied by name, and a field
  whose size changed refuses the conversion). The per-player fields above are remapped,
  the four new slots and houses are cleared with the game's own routines afterwards
  (`pc_save_convert_fixup_loaded`), except the gyroid messages, which need the ROM and are
  loaded on the first play frame (`pc_save_convert_late_fixup`, from `play_main`) -- the
  ROM loader's heaps do not exist at load time and `mHm_ClearHomeInfo` crashed there.
  The old file is kept once as `DobutsunomoriP_MURA.gci.before-eight`. Tested on the rig:
  the old test town loaded, played, saved (0xA2040) and uploaded; the server logged "now
  stored in the eight-resident layout".
- *The server* (`town.c`) accepts a stored 0x72040 town, serves it as it is, reads slots
  0-3 at the old offsets (`ACNET_LEGACY_*` in protocol.h), has no blocks for slots 4-7,
  refuses resident pushes into it ("still a four-resident file"; relayed only), and
  replaces it on the first upload. Protocol version 7, `ACNET_MAX_PLAYERS` 8,
  `ACNET_MAX_CLIENTS` 16. `m_puppet.c_inc` asserts every layout constant at compile time.
  `mktown.py`/`make_resident.py` read the new constants (`ACNET_LAND_INFO_OFFSET`,
